...
export default function App() {
  ...
  useEffect(() => {
    const savedTasks = localStorage.getItem('macaron_tasks');
    const savedCategories = localStorage.getItem('macaron_categories');
...
function TaskItem({ task, category, onComplete, onDelete }: TaskItemProps) {
  return (
    <motion.div 
      layout
      ...
      className="bg-white p-6 rounded-3xl shadow-md border-2 border-macaron-pink flex items-center gap-5 group relative overflow-hidden"
    >
      <div 
...
function TaskFormModal({ categories, onClose, onSubmit, onAddCategory }: { 
  categories: Category[]; 
  onClose: () => void; 
      ...
      >
        <div className="p-8 space-y-8">
          <div className="flex items-center justify-between">
            <h3 className="text-3xl font-medium text-slate-700">新增任務</h3>
          ...
          <button 
            ...
            onClick={() => onSubmit(title, categoryId, hours, minutes)}
...
function CompleteTaskModal({ task, onClose, onSubmit }: { 
  ...
  useEffect(() => {
    setHours(Math.floor(task.estimatedMinutes / 60));
    setMinutes(task.estimatedMinutes % 60);
  }, [task]);
